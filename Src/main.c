...
int main(void)
{
 ...
 // Habilitar la recepción UART por interrupción por primera vez
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    // Desactivo la parte del LED de latido por que interfiere con el LED externo
      // ---- Heartbeat LED ----
    /* static uint32_t last_heartbeat_time = 0;
    if (HAL_GetTick() - last_heartbeat_time >= 100) // Cada 500 ms : cambiado LD2 a 100 ms 
    {
    ...
    if (button_flag == 1)
    {
      ...
      // 2. Calcular cuándo apagarlo (3 segundos desde ahora) LED EXTERNO
      led_ext_off_time = HAL_GetTick() + 1000; // Modoficacion: cambiado led externo a 1000 ms
      
      // 3. Enviar mensaje por UART
      sprintf(tx_buffer, "Hola.. Boton B1 presionado! LED EXT ON.\r\n"); // Modificacion de mobre cada que se presiona el boton B1
    ...
    if (led_ext_off_time != 0 && HAL_GetTick() >= led_ext_off_time)
    {
      ...
      sprintf(tx_buffer, "Luces apagadas. (timeout).\r\n");
      HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
